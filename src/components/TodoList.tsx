...
import { Plus, Search, Layout, RefreshCw, LogOut, FolderOpen, CheckSquare } from 'lucide-react';
...
export function TodoList({ cred, onChangeProject, onLogout }: TodoListProps) {
  ...
  const { todos, loading, error, refresh, moveIssue, updateTitle, updateStoryPoints } = useJiraBoard(cred, cred?.projectKey ?? null);
  ...
  const addTodo = useCallback(
    async (data: { title: string; description?: string; type: import('@/types/todo').IssueType; priority: import('@/types/todo').Priority; status: IssueStatus }) => {
      setCreateError(null);
      try {
  ...
  const filteredTodos = useMemo(() => todos.filter((t) => t.title.toLowerCase().includes(searchQuery.toLowerCase()) || t.issueKey.toLowerCase().includes(searchQuery.toLowerCase())), [todos, searchQuery]);
