...
export function useMySubtasks(cred: JiraCredentials | null, projectKey: string | null) {
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const issues = await jiraApi.getMySubtasks(cred, projectKey);
  ...
  const moveSubtask = useCallback(
    async (key: string, targetStatus: IssueStatus) => {
      ...
      setGroups((prev) =>
        prev.map((group) => ({
          ...group,
          subtasks: group.subtasks.map((t) => (t.key === key ? { ...t, status: targetStatus, statusName: statusToJiraName[targetStatus] } : t)),
